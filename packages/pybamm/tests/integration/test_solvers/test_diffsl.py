...
class TestDiffSLExport:
    ...
    @pytest.mark.parametrize(
        "n_points, tol",
        # small table -> piecewise-linear (near-exact vs pybamm's own
        # linear interpolation); large table -> Chebyshev approximation,
        # which pybamm does not use, so allow a looser fit tolerance.
        [(8, 1e-5), (60, 5e-3)],
        ids=["piecewise_linear", "chebyshev"],
    )
    def test_interpolant_solve_matches_pybamm(self, n_points, tol):
        # Numerical guard on the Interpolant export paths (the unit tests
        # only check structure): export an ODE whose RHS is a data
        # Interpolant, solve via diffsol, and compare the trajectory to an
        # independent pybamm CasadiSolver solve of the identical model.
        import pydiffsol as ds

        x_data = np.linspace(0.0, 1.0, n_points)
        ...
        np.testing.assert_allclose(y_diffsol, y_pybamm, rtol=tol, atol=tol)
